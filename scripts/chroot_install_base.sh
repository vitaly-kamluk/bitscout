...
apt-fast -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install  binutils netcat socat casper lupin-casper discover laptop-detect os-prober lxc lxc1 bindfs dialog tmux gawk grub-pc ntpdate  && exit 0 || exit 1' || exit 1
else
  chroot_exec chroot 'export DEBIAN_FRONTEND=noninteractive
apt-fast -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install  binutils file hdparm iptables lshw usbutils parted lsof psmisc strace ltrace time systemd-sysv man-db dosfstools cron busybox-static rsync dmidecode bash-completion command-not-found ntfs-3g netcat socat uuid-runtime vim nano less pv casper lupin-casper discover laptop-detect os-prober lxc lxc1 bindfs wicd-curses dialog tmux gawk ntpdate nbd-server grub-pc   qemu-kvm  && exit 0 || exit 1' || exit 1
fi &&
