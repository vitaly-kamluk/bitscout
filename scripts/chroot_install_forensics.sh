...
case $GLOBAL_RELEASESIZE in
 1)
   chroot_exec 'DEBIAN_FRONTEND=noninteractive apt-get --yes install dd hexedit'
   ;;
 2)
   chroot_exec 'DEBIAN_FRONTEND=noninteractive apt-get --yes install dcfldd sleuthkit plaso hexedit'
   ;;
 3)
